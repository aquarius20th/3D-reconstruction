...
int main(int, char ** argv)
{
	...
	{
		char str[30];
		sprintf(str, "E:\\v1_100\\%d.tif", i);
		IplImage *src = cvLoadImage(str, 0);
		for (int j = 0; j < 100; j++)
		{
			uchar *ptr = (uchar*)src->imageData + j * 100;
			for (int cols = 0; cols < 100; cols++)
			{
				if (ptr[cols] >= 5)
				{
					VolumeData[i*dimY*dimX + j * 4 * dimX + cols * 4] = 1;
				}
				else
				{
					VolumeData[i*dimY*dimX + j * 4 * dimX + cols * 4] = 0;
				}
...
/*	for (int i = 1; i < 400; i++)
	{
		char str[30];
		sprintf(str, "E:\\v3_flip_100\\%d.tif", 400 - i);
		IplImage *src = cvLoadImage(str, 0);
		for (int j = 0; j <100; j++)
		{
			uchar *ptr = (uchar*)src->imageData + j * 100;
			for (int cols = 0; cols <100; cols++)
			{
				m[0] = (double)cols, m[1] = (double)j, m[2] = (double)i;
				int a, b, c;
				/*mul = m*p*p2;


				
				m1[0] = mul[0] / 2, m1[1] = mul[1] / 2;
				mul1 = m1*s;
				mul[0] = mul1[0] + mul[0] / 2, mul[1] = mul1[1] + mul[1] / 2;
				mul[0] -= 7.3,mul[1]+=2,mul[2]+=2.8;
				printf("%lf %lf %lf\n", mul[0], mul[1], mul[2]);
				a = (int)(4 * mul[0] + 512) % 512, b = (int)(4 * mul[1] + 512) % 512, c = (int)(mul[2] + 512) % 512;
				printf("a=%d b=%d c=%d\n", a, b, c);
				if (ptr[cols] >= 5)
				{
					VolumeData[c*dimY*dimX + b*dimX + a] |= ptr[cols];
				}
				else
				{
	...
	for (int i = 1; i <=400; i++)
	{
		...
		for (int j = 0; j < 100; j++)
		{
			...
			for (int cols = 0; cols < 100; cols++)
			{
				if (ptr[cols] >= 5)
				{
					VolumeData[(400-4*cols-3)*dimY*dimX + 4*j*dimX + i] |= ptr[cols];
				}
				else
				{
					VolumeData[(400-4*cols-3)*dimY*dimX + 4*j*dimX + i]|= 0;
				}
